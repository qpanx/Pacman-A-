...
class Ghost:
    ...
    def move_astar(self):
        """Move ghost using A* pathfinding algorithm"""
        ...
        open_set = [(0, start, self.direction, [])]  
        ...
        while open_set and iterations < max_iterations:
            ...
            f_score, current, current_direction, path = heapq.heappop(open_set)
            
            
            if manhattan_distance(current, goal) < 30:
            ...
            for next_dir in range(4):  
                ...
                ghost_copy = Ghost(next_x-22, next_y-22, self.target, self.speed, 
                                  self.img, next_dir, self.dead, self.in_box, self.id)
                turns, _ = ghost_copy.check_collisions()
                
                if not turns[next_dir]:
                    continue
                
                
                g_score = g_scores[current] + 1
                
                if g_score < g_scores[next_pos]:
                    
                    g_scores[next_pos] = g_score
                    f_score = g_score + manhattan_distance(next_pos, goal)
                    
                    
                    new_path = path + [next_dir]
                    heapq.heappush(open_set, (f_score, next_pos, next_dir, new_path))
